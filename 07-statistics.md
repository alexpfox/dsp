...
>>> We know he's a twin, and we know they were both boys. Identical twins must necessarily be same sex, where fraternal twins can include both sexes. All twin sex pairings are MM, FM, MF, or FF, so twin boys = 1/4. This gives us a bit more information to work off of.

>>>Bayes' theorem can be rephrased as "the posterior probability equals the prior probability times the likelihood ratio." or P(A|B) = P(A)*P(B|A)/P(B)

>>>A = identical twins B = Elvis was a twin

>>>P(fraternal boys) = P(fraternal) * P(boys) = (1/125) * (1/4) P(identical boys) = P(identical) * P(boys) = (1/300) * (1/2)

>>>P(identical) = P(identical boys) / (P(identical boys) + P(fraternal boys))
...
>> Frequentist statisticians are more limited in a sense than bayesian, as they consider only the raw potential for an outcome based on what's known initially about a problem and the potential results. They don't consider the historical data for outcomes in predicting further results, which can often alter statistical interpretation of a situation. 
